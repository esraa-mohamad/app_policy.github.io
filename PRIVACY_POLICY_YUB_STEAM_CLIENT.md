# 🔒 Privacy Policy — YUB Steam Client App

> **Last Updated:** May 16, 2026

Welcome to **YUB Steam Client App** : **YUB Steam - يوب ستيم** .  
Your privacy and data security are important to us. This Privacy Policy explains how we collect, use, protect, and manage your information when you use our mobile application and services.

🌐 **Official Email:**  
https://app.yubsteam.com

By downloading, accessing, or using the App, you agree to the practices described in this Privacy Policy.

---

# 📌 Table of Contents

- [Introduction](#-introduction)
- [Information We Collect](#-information-we-collect)
- [How We Use Your Information](#-how-we-use-your-information)
- [Data Sharing & Disclosure](#-data-sharing--disclosure)
- [Data Storage & Security](#-data-storage--security)
- [Your Rights & Choices](#-your-rights--choices)
- [Permissions We Request](#-permissions-we-request)
- [Third-Party Services](#-third-party-services)
- [Children's Privacy](#-childrens-privacy)
- [International Data Transfers](#-international-data-transfers)
- [Policy Updates](#-changes-to-this-privacy-policy)
- [Contact Information](#-contact-us)
- [Compliance](#-compliance)

---

# 📖 Introduction

YUB Steam ("we", "our", or "us") is committed to protecting your personal information and maintaining transparency regarding how your data is handled.

This Privacy Policy applies to:

- 📱 YUB Steam Client Mobile Application
- 🌐 Services provided through `app.yubsteam.com`
- 🛠 Related support and operational services

If you do not agree with this policy, please discontinue use of the App.

---

# 📥 Information We Collect

## 👤 Personal Information

We may collect the following information directly from you:

### 📝 Account Registration
- Full name
- Phone number
- Country code
- Account type (Individual / Company)
- Company details
- Commercial registration
- Tax information
- Uploaded verification documents

### 🔐 Authentication
- OTP verification data
- Session authentication tokens

> ⚠️ We do **not** use password-based authentication for customers.

### 🚗 Vehicle Information
- Vehicle brand & model
- Vehicle category
- Color
- Plate number
- Uploaded vehicle images

### 📍 Address Information
- Address labels
- Detailed address information
- GPS coordinates selected through maps

### 📅 Booking Information
- Service packages
- Scheduled appointments
- Notes & instructions
- Coupon codes
- Pricing details
- Booking history

### ⭐ Ratings & Feedback
- Service ratings
- User feedback and reviews

### 🛠 Support Requests
- Complaint submissions
- Technical support messages
- Attached screenshots or images

### 💳 Payment Information
Payments are securely processed by third-party providers.

> 🔒 We do **not** store full payment card details on our servers.

---

# ⚙️ Automatically Collected Information

When using the App, certain information may be collected automatically:

- Device type & operating system
- Unique device identifiers
- App version
- Usage statistics
- Crash logs & diagnostics
- IP address & API logs
- Firebase Cloud Messaging (FCM) notification tokens

---

# 📍 Location Information

We may collect GPS location data when you:

- Select service locations
- Confirm addresses on maps
- Request location-based services

Location data is used strictly for operational and booking purposes.

---

# 🔗 Third-Party Services Information

Our App integrates with trusted third-party services including:

| Service | Purpose |
|---|---|
| Firebase | Notifications & App Infrastructure |
| Google Maps | Maps & Address Selection |
| Payment Gateways | Secure Payment Processing |

---

# 🛠 How We Use Your Information

We use collected information to:

## ✅ Provide Services
- Create and manage accounts
- Process bookings
- Schedule appointments
- Manage addresses & vehicles
- Generate invoices and history

## 📢 Communication
- Send booking notifications
- Deliver reminders and updates
- Respond to support requests

## 📊 Analytics & Improvements
- Improve app functionality
- Analyze usage trends
- Detect fraud or abuse
- Enhance performance & stability

## ⚖️ Legal Compliance
- Comply with applicable laws
- Enforce Terms & Conditions
- Protect platform integrity

---

# 🤝 Data Sharing & Disclosure

We do **not sell** your personal information.

## We may share data with:

### 🧩 Service Providers
- Firebase (Google)
- Google Maps
- Payment processors
- Cloud hosting providers

### 🚘 Field Workers & Operations Team
To fulfill your booking, assigned workers may access:
- Name & phone number
- Address & GPS coordinates
- Vehicle details
- Booking notes
- Appointment schedules

### ⚖️ Legal Authorities
Where required by applicable law or valid legal request.

### 🔄 Business Transfers
In cases of merger, acquisition, or asset sale.

---

# 🔐 Data Storage & Security

## 🛡 Security Measures

We implement industry-standard protections including:

- HTTPS/TLS encryption
- Secure token authentication
- Restricted server access
- Regular security updates

## 🗂 Data Retention

Your data is retained only as long as necessary for:
- Service delivery
- Legal obligations
- Dispute resolution
- Security purposes

---

# 👤 Your Rights & Choices

## ✏️ Access & Updates
You may update:
- Profile information
- Saved vehicles
- Addresses

directly from the App.

---

## 📍 Location Permissions
Location access can be enabled or disabled from your device settings.

---

## 🔔 Notifications
Push notifications can be managed through:
- App settings
- Device notification settings

---

## ❌ Account Deletion
You may request account deletion through the App settings.

Some data may remain stored where legally required.

---

## 📦 Data Portability
You may request a copy of your personal data by contacting us.

---

# 👶 Children's Privacy

Our services are not intended for users under the age of 13 (or the minimum legal age in your region).

We do not knowingly collect children's data.

---

# 📱 Permissions We Request

| Permission | Purpose |
|---|---|
| Camera | Upload vehicle/support images |
| Photos & Storage | Select gallery images |
| Location | Address & map selection |
| Notifications | Booking alerts |
| Internet | API communication |

You may revoke permissions anytime via device settings.

---

# 🌍 International Data Transfers

Your information may be processed outside your country of residence.

We apply appropriate safeguards to protect transferred data according to applicable laws.

---

# 🔄 Changes to This Privacy Policy

We may update this Privacy Policy periodically.

Updates will be reflected by:
- Revising the "Last Updated" date
- Publishing updated versions inside the App

---

# ✅ Your Consent

By using the YUB Steam Client App, you agree to this Privacy Policy and our data practices.

---

# 📫 Contact Us

If you have any questions or privacy-related requests:

## 📧 Email Support
- technology.organizationsa@gmail.com
- esoodawood186@gmail.com

## 🛠 In-App Support
Use the support & complaints section inside the App.

---

# ⚖️ Compliance

This Privacy Policy is designed to align with:

- ✅ Google Play Privacy Requirements
- ✅ GDPR (General Data Protection Regulation)
- ✅ CCPA (California Consumer Privacy Act)
- ✅ Applicable international privacy laws

---

# 🏢 Data Controller Information

| Item | Details |
|---|---|
| App Name | YUB Steam Client |
| Bundle ID | `yub.client.techorg` |
| Developer | Esraa Mohamad AbdelHafiz / TechOrg |

---

<p align="center">
  🔒 Your privacy matters to us.
</p>

<p align="center">
  Made with ❤️ by <strong>Dexterous Flutter Team in TechOrg</strong>
</p>

---

> This Privacy Policy applies to all users of the YUB Steam Client App and is effective as of the date listed above.
